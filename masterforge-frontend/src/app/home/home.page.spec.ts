...
describe('HomePage — Task 2: Preservation — DM Summary Cards and Behavior Unchanged', () => {
  ...
  function buildTestBed(user: any) {
    ...
    apiSpy.getSessions.and.returnValue(of([
      { id: '1', scheduledDate: '2024-06-15T18:00:00Z', price: 25, campaign: null },
    ]));
    apiSpy.getPlayerCount.and.returnValue(of({ playerCount: 5 }));
  ...
  // -------------------------------------------------------------------------
  // DM summary cards preservation: Próxima Sesión, Clientes Totales, Ingresos del Mes
  // Validates: Requirement 3.2
  // **Validates: Requirements 3.2**
  //
  // EXPECTED ON UNFIXED CODE: PASSES (summary cards are already rendered correctly)
  // -------------------------------------------------------------------------
  it('DM summary cards preservation: Próxima Sesión card is rendered with correct data', async () => {
    await buildTestBed({ id: 'user-1', name: 'DM User' });
    fixture = TestBed.createComponent(HomePage);
    component = fixture.componentInstance;
    roleSubject.next('dm');
    fixture.detectChanges();

    const dmDashboard: HTMLElement | null = fixture.nativeElement.querySelector('[data-testid="dm-dashboard"]');
    expect(dmDashboard).withContext('[data-testid="dm-dashboard"] should exist in DM mode').toBeTruthy();

    const summaryCards = dmDashboard!.querySelectorAll('.summary-card');
    expect(summaryCards.length).withContext('Should have at least 3 summary cards').toBeGreaterThanOrEqual(3);

    // Check for "Próxima Sesión" card
    const cardTexts = Array.from(summaryCards).map(card => card.textContent?.trim() ?? '');
    const hasProximaSesion = cardTexts.some(text => text.includes('Próxima Sesión'));
    expect(hasProximaSesion).withContext('Should have "Próxima Sesión" summary card').toBeTrue();
  });

  it('DM summary cards preservation: Clientes Totales card is rendered with correct data', async () => {
    await buildTestBed({ id: 'user-1', name: 'DM User' });
    fixture = TestBed.createComponent(HomePage);
    component = fixture.componentInstance;
    roleSubject.next('dm');
    ...
    const dmDashboard: HTMLElement | null = fixture.nativeElement.querySelector('[data-testid="dm-dashboard"]');
    expect(dmDashboard).toBeTruthy();

    const summaryCards = dmDashboard!.querySelectorAll('.summary-card');
    const cardTexts = Array.from(summaryCards).map(card => card.textContent?.trim() ?? '');
    const hasClientesTotales = cardTexts.some(text => text.includes('Clientes Totales'));
    expect(hasClientesTotales).withContext('Should have "Clientes Totales" summary card').toBeTrue();
  });

  it('DM summary cards preservation: Ingresos del Mes card is rendered with correct data', async () => {
    await buildTestBed({ id: 'user-1', name: 'DM User' });
    fixture = TestBed.createComponent(HomePage);
    component = fixture.componentInstance;
    roleSubject.next('dm');
    ...
    const summaryCards = dmDashboard!.querySelectorAll('.summary-card');
    const cardTexts = Array.from(summaryCards).map(card => card.textContent?.trim() ?? '');
    const hasIngresos = cardTexts.some(text => text.includes('Ingresos'));
    expect(hasIngresos).withContext('Should have "Ingresos" summary card').toBeTrue();
  });
  ...
  it('PBT — balance getter safety: for any user object, accessing balance never throws and returns a number (or undefined on unfixed code)', async () => {
    // Use a mutable holder so we can change the user without re-creating TestBed
    const userHolder = { current: null as any };
    ...
    const authServiceMock = {
      getCurrentUser: () => userHolder.current,
      getUserIdFromToken: () => 'user-1',
      logout: () => {},
    };
    ...
    fc.assert(
      fc.property(
        fc.oneof(
          fc.constant(null),
          fc.record({ id: fc.uuid(), name: fc.string({ minLength: 1 }), balance: fc.float({ noNaN: true }) })
        ),
        (user) => {
  ...
  // -------------------------------------------------------------------------
  // PBT — Role toggle stability: DM summary cards unaffected by role toggle
  // Validates: Requirement 3.3
  // **Validates: Requirements 3.3**
  //
  // EXPECTED ON UNFIXED CODE: PASSES (summary cards are independent of role toggle sequence)
  // -------------------------------------------------------------------------
  it('PBT — role toggle stability: for any sequence of toggleRole() calls, DM summary cards remain correct when returning to DM mode', async () => {
    await buildTestBed({ id: 'user-1', name: 'DM User' });
    fixture = TestBed.createComponent(HomePage);
    component = fixture.componentInstance;
    fixture.detectChanges();
    ...
    fc.assert(
      fc.property(
        ...
        (toggles) => {
          ...
          // Return to DM mode if needed
          if (roleSubject.value !== 'dm') {
            roleSubject.next('dm');
            fixture.detectChanges();
          }

          // DM summary cards should still be present and correct
          const dmDashboard: HTMLElement | null = fixture.nativeElement.querySelector('[data-testid="dm-dashboard"]');
          expect(dmDashboard).withContext('DM dashboard should be visible in DM mode').toBeTruthy();

          const summaryCards = dmDashboard!.querySelectorAll('.summary-card');
          expect(summaryCards.length).withContext('Should have at least 3 summary cards').toBeGreaterThanOrEqual(3);

          const cardTexts = Array.from(summaryCards).map(card => card.textContent?.trim() ?? '');
          expect(cardTexts.some(text => text.includes('Próxima Sesión'))).toBeTrue();
          expect(cardTexts.some(text => text.includes('Clientes Totales'))).toBeTrue();
          expect(cardTexts.some(text => text.includes('Ingresos'))).toBeTrue();
        }
      ),
      { numRuns: 30 }
    );
...
// ---------------------------------------------------------------------------
// Task 1 — Bug Condition Exploration: Balance NOT Rendered in DM Dashboard
// ---------------------------------------------------------------------------
// These tests MUST FAIL on unfixed code — failure confirms the bug exists.
//
// Bug condition: DM user is authenticated, getCurrentUser() returns an object
// with `balance`, but the DM dashboard does NOT render the formatted balance.
//
// **Validates: Requirements 1.2**
// ---------------------------------------------------------------------------

describe('HomePage — Task 1: Bug Condition — Balance Not Rendered in DM Dashboard', () => {
  ...
  // -------------------------------------------------------------------------
  // Test 2 — DM dashboard should contain "€100.00" when user has balance 100
  // EXPECTED ON UNFIXED CODE: FAILS
  // Counterexample: [data-testid="dm-dashboard"] has no balance card with "€100.00"
  // **Validates: Requirements 1.2**
  // -------------------------------------------------------------------------
  it('Test 2 — [data-testid="dm-dashboard"] should contain "€100.00" when user has balance 100', async () => {
    await buildTestBed({ id: 'user-1', name: 'DM User', balance: 100 });
    fixture = TestBed.createComponent(HomePage);
    component = fixture.componentInstance;
    roleSubject.next('dm');
    fixture.detectChanges();
    ...
    const dashboardText = dmDashboard!.textContent ?? '';
    const hasBalance = dashboardText.includes('€100.00') || dashboardText.includes('100.00');
    expect(hasBalance)
      .withContext(
        `[BUG COUNTEREXAMPLE] dm-dashboard text is "${dashboardText.trim()}" — does not contain "€100.00"`
      )
      .toBeTrue();
  });

  // -------------------------------------------------------------------------
  // Test 3 — Zero balance: balance = 0 → "€0.00" should appear in DM dashboard
  // EXPECTED ON UNFIXED CODE: FAILS
  // Counterexample: [data-testid="dm-dashboard"] has no element with "€0.00"
  // **Validates: Requirements 1.2**
  // -------------------------------------------------------------------------
  it('Test 3 — [data-testid="dm-dashboard"] should contain "€0.00" when user has balance 0', async () => {
    await buildTestBed({ id: 'user-1', name: 'DM User', balance: 0 });
    fixture = TestBed.createComponent(HomePage);
    component = fixture.componentInstance;
    roleSubject.next('dm');
    fixture.detectChanges();
    ...
    const dashboardText = dmDashboard!.textContent ?? '';
    const hasBalance = dashboardText.includes('€0.00') || dashboardText.includes('0.00');
    expect(hasBalance)
      .withContext(
        `[BUG COUNTEREXAMPLE] dm-dashboard text is "${dashboardText.trim()}" — does not contain "€0.00"`
      )
      .toBeTrue();
  });
