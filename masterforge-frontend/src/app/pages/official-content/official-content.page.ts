...
@Component({
  ...
  templateUrl: './official-content.page.html',
  styleUrls: ['./official-content.page.scss'],
  standalone: true,
  imports: [
    IonSpinner, IonList, IonItem, IonLabel, IonButton,
    IonModal, IonContent, IonIcon,
...
export class OfficialContentPage implements OnInit {

  officialItems: HomebrewSummary = {
  ...
  ngOnInit() {
    ...
    this.resetLimits();
    this.loadOfficialContent();
  }

  loadOfficialContent(): void {
    this.loading = true;
    this.error = null;
    this.homebrewService.getOfficialHomebrew().subscribe({
      next: (data) => {
        this.officialItems = data;
        this.loading = false;
      },
      error: (err) => {
        console.error('Error al cargar contenido oficial', err);
        this.error = err?.message ?? 'Error al cargar el contenido oficial del sistema';
        this.loading = false;
      }
    });
  }
  ...
  getFilteredItemsFull(key: string): HomebrewItem[] {
    const items = this.officialItems[key as keyof HomebrewSummary] || [];
  ...
  navigateToCreate(type: ContentType): void {
    ...
    this.router.navigate([routeMap[type]], { queryParams: { from: 'official' } });
  }
  ...
  navigateToEdit(type: ContentType, id: string): void {
    ...
    this.router.navigate([editMap[type]], { queryParams: { from: 'official' } });
  }

  confirmDelete(item: HomebrewItem): void {
    const confirmed = window.confirm(`¿Estás seguro de que quieres eliminar este contenido oficial "${item.name}"?`);
    if (confirmed) {
  ...
  deleteItem(item: HomebrewItem): void {
    ...
    this.homebrewService.deleteItem(item.contentType, item.id).subscribe({
      next: () => {
        ...
        this.officialItems = {
          ...this.officialItems,
          [key]: this.officialItems[key].filter(i => i.id !== item.id),
        };
        this.deletingId = null;
        this.notificationService.showSuccess('Contenido oficial eliminado correctamente.');
      },
      error: (err) => {
        console.error('Error al eliminar el contenido oficial', err);
        this.notificationService.showError(err?.error?.message ?? err?.message ?? 'Error al eliminar el contenido oficial');
        this.deletingId = null;
      }
    });
  }
